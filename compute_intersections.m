...
function [XintersectionMatrix,YintersectionMatrix,IntMatrix,rayAng] = compute_intersections(geo)
...
%                YintersectionMatrix(i,2) = ray(3)*(det - ray(1)) + ray(2);
               ...
               if discrim > 0
                   ...
                   aInc = atan((tan(aRef) - (((aEll2^2)*(yint-kFace2))/((geo.bEll2^2)*(xint-hFace2)))) / (1 + (((aEll2^2)*(yint-kFace2))/((geo.bEll2^2)*(xint-hFace2)))*tan(aRef)));
  ...
  % rays that go straight to Detectors
    ...
    XintersectionMatrix(i,9) = det;
    YintersectionMatrix(i,9) = ray(3)*(det - ray(1)) + ray(2);
    rayAng(i,9) = angles(i);
